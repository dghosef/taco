...
def makemap(fns):
    ...
    cpp = Cppgen()
    for fn in fns:
        call_params = ["PochiVM::StaticCast<" + i[0] + ">(params[" + str(i[1]) + "])" for i in zip(fn[2], range(len(fn[2])))]
        body = cpp.call("PochiVM::CallFreeFn::" + fn[1] + "_wrapper", call_params)
        body = cpp.retstmt(body)
        values.append(cpp.lmbda("PochiVM::ValueVT", ["std::vector<PochiVM::ValueVT> &params"], body))
...
if __name__ == "__main__":
    ...
    write_cpp("./src/codegen/callables.h", makemap(fns), ["\"pochivm.h\"", "<map>", "<string>", "<functional>"])
