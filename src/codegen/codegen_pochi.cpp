...
// #define CODEGEN_POCHI_LOG
using namespace std;

static const bool usePochi = true;
namespace taco {
bool should_use_pochi_codegen(std::string fnName) {
  if (fnName == "iterate" || fnName == "_shim_iterate" || fnName == "pack" ||
      fnName == "_shim_pack" || fnName == "" || !usePochi) {
    return false;
  } else {
    return true;
  }
...
const string cHeaders =
    ...
    "}\n"
    "int taco_binarySearchAfter(int *array, int arrayStart, int arrayEnd, int "
    "target) {\n"
    ...
    "  return upperBound;\n"
    "}\n"
    "int taco_binarySearchBefore(int *array, int arrayStart, int arrayEnd, int "
    "target) {\n"
    ...
    "  t->mode_types    = (taco_mode_t *) malloc(order * "
    "sizeof(taco_mode_t));\n"
...
class CodeGen_Pochi::FindVars : public IRVisitor {
public:
  ...
  CodeGen_Pochi *codeGen;

  // copy inputs and outputs into the map
  FindVars(vector<Expr> inputs, vector<Expr> outputs, CodeGen_Pochi *codeGen)
      : codeGen(codeGen) {
    for (auto v : inputs) {
      auto var = v.as<Var>();
...
CodeGen_Pochi::CodeGen_Pochi(std::ostream &dest, OutputKind outputKind,
                             std::string fnName, bool simplify)
    : CodeGen(dest, false, simplify, C), out(dest), outputKind(outputKind),
      compiler(fnName) {}

CodeGen_Pochi::~CodeGen_Pochi() {}

void CodeGen_Pochi::dump(std::string msg) {
#ifdef CODEGEN_POCHI_LOG
...
void CodeGen_Pochi::compile(Stmt stmt, bool isFirst) {
  varMap = {};
  localVars = {};
  compiler.visit_taco_op(stmt);
}

void CodeGen_Pochi::visit(const Function *func) {
...
void CodeGen_Pochi::visit(const VarDecl *op) {
  dump("var");
  if (emittingCoroutine) {
    doIndent();
    op->var.accept(this);
...
void CodeGen_Pochi::visit(const Yield *op) {
  dump("yield");
...
void CodeGen_Pochi::visit(const Var *op) {
  dump("var " + op->name);
...
static string getAtomicPragma() { return "#pragma omp atomic"; }
...
void CodeGen_Pochi::visit(const For *op) {
  dump("for");
  switch (op->kind) {
...
void CodeGen_Pochi::visit(const While *op) {
...
void CodeGen_Pochi::visit(const GetProperty *op) {
  dump("getprop");
...
void CodeGen_Pochi::visit(const Min *op) {
  dump("min");
...
void CodeGen_Pochi::visit(const Max *op) {
  dump("max");
  if (op->operands.size() == 1) {
    op->operands[0].accept(this);
    return;
...
void CodeGen_Pochi::visit(const Allocate *op) {
  dump("allocate");
...
void CodeGen_Pochi::visit(const Sqrt *op) {
  dump("sqrt");
...
void CodeGen_Pochi::visit(const Assign *op) {
  dump("assign");
  if (op->use_atomics) {
    doIndent();
...
void CodeGen_Pochi::visit(const Store *op) {
  dump("store");
  if (op->use_atomics) {
    doIndent();
    stream << getAtomicPragma() << endl;
...
void CodeGen_Pochi::generateShim(const Stmt &func, stringstream &ret) {
  ...
  for (auto output : funcPtr->outputs) {
    ...
    auto cast_type =
        var->is_tensor ? "taco_tensor_t*" : printCType(var->type, var->is_ptr);
