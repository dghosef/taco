...
PochiVM::ValueVT PochiVTCompiler::visit_pochi(const taco::ir::Min *op) {
  if (op->operands.size() == 1) {
    return visit_taco_op(op->operands[0]);
  }
  auto arg1 = visit_taco_op(op->operands[0]);
  auto arg2 = visit_taco_op(op->operands[1]);
  return PochiVM::CallFreeFn::min(PochiVM::StaticCast<int>(arg1),
                                  PochiVM::StaticCast<int>(arg2));
}
//
PochiVM::ValueVT PochiVTCompiler::visit_pochi(const taco::ir::Max *op) {
  ...
  auto arg1 = visit_taco_op(op->operands[0]);
  auto arg2 = visit_taco_op(op->operands[1]);
  return PochiVM::CallFreeFn::max(PochiVM::StaticCast<int>(arg1),
                                  PochiVM::StaticCast<int>(arg2));
}
...
PochiVM::ValueVT PochiVTCompiler::visit_pochi(const taco::ir::For *op) {
  ...
  if (lit != nullptr && ((lit->type.isInt() && lit->equalsScalar(1)) ||
    ...
    auto inc_stmt = PochiVM::Assign(var, val);
...
PochiVM::ValueVT PochiVTCompiler::visit_pochi(const taco::ir::Function *func) {
  ...
  body.Append(visit_taco_op(func->body));
  ...
  indent_guard::reset();
  debugPrint("\n" + pochiToStr(PochiVM::Value<void>(body).__pochivm_value_ptr),
             false);
  fn.SetBody(body);
